...
This repository and its `wasm-ld` release assets use LLVM's [Apache-2.0 license with LLVM Exceptions](LICENSE.txt). Each release archive includes the pinned upstream license and a build receipt.

## Windows ARM64 wasm-ld

[`eng/toolchain.json`](eng/toolchain.json) is the one place to update the LLVM revision, selected source-tree digest, build flags, license digest, and next release tag. The [release workflow](.github/workflows/release.yml) runs only when started manually from `main`. It builds on a native Windows ARM64 runner, verifies the PE machine type and version, packages the executable with its path-free build receipt and LLVM license, checks the uploaded archive, then publishes the GitHub Release. A failed build never publishes a release; a failed upload check leaves a draft for inspection.
